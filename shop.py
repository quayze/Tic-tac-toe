...
class Shop(Drawable):
    def __init__(self, game_session : GameSession, game):
        ...
        self.reroll_price = 5
        self.close_but = Button(self.center + Vector2(500, 0), 300, 500, (200, 0, 0), text= ['Next', 'Game'])
        ...
        self.p1_pay_surface = pygame.rect.Rect((0, 0, * p1_cases_size))
        self.p1_pay_surface.center = p1_cases_pos


    def _pay_item(self):
        current_item = self.case_area.get_selected()
        if self.p1_pay_surface.colliderect(current_item.get_rect()):
            inventory = self.session.get_inventory(self.player1)
    ...
    def reroll_items(self):
        ...
        for i in range(self.max_cases):
            ...
            item = Item(self.center + Vector2(i, 0), width= 100, height= 100, object= case)
